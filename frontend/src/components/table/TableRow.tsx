  ...
  return (
    <tr
      className={cn(
        ...
        effectiveStatus === "completed" && "bg-emerald-50/30 dark:bg-emerald-900/20",
        paddingClasses
      )}
    >
      {/* Status */}
      <td className={cn("px-3 sm:px-4 min-w-[200px] w-[200px]", hasProgressInfo && "align-top")}>
      ...
      <td className="px-3 sm:px-4">
        <Tooltip content={buildScanTooltipContent(scan)}>
          <button
            className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-1 rounded transition-colors h-10 w-10 min-h-[40px] min-w-[40px] flex items-center justify-center"
